...
function manipulateQuery(url, options) {
  ...
  if (options && options.append && typeof options.append === 'object') {
 
    for (const [key, value] of Object.entries(options.append)) {
    
      urlObj.searchParams.append(key, value);
    }
  } if (options && options.delete && Array.isArray(options.delete)) {
    
    for (const keyToDelete of options.delete) {
...
function sortUrlParams(url) {
  ...
  const urlObj = new URL(url);
  const paramsObject = Object.fromEntries(urlObj.searchParams.entries());
  const newUrlObj = new URL(url);

  newUrlObj.search = new URLSearchParams(Object.entries(paramsObject).sort());
...
function getURLValues(url) {
  ...
  const urlObj = new URL(url);
  const valuesArray = Array.from(urlObj.searchParams.values());
